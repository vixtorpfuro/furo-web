...
export default function Contacto() {
  ...
  const submit = async () => {
    if (!form.nombre || !form.apellido || !form.email || !form.celular || !form.soy || !form.cuando || !form.como_contactar || !form.privacidad) {
  ...
  return (
    <main style={{ backgroundColor: '#f5f3ee', color: '#141210', fontFamily: 'DM Sans, sans-serif', minHeight: '100vh' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;700&display=swap');*{margin:0;padding:0;box-sizing:border-box;}a{text-decoration:none;} input:focus,select:focus{border-color:#52448a !important;} input::placeholder{color:#c8c4bc;}`}</style>
      <Nav dark/>

      {/* HEADER */}
      <div style={{ paddingTop: 120, padding: '120px 56px 72px' }}>
        <FadeUp>
          ...
          <h1 style={{ fontSize: 'clamp(42px,6vw,40px)', fontWeight: 300, lineHeight: 1.05, color: '#141210', marginBottom: 40 }}>Hablemos de tu proyecto</h1>
      ...
      {/* FORM + ASIDE */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', borderTop: '1px solid rgba(20,18,16,0.08)' }}>

        {/* FORMULARIO */}
        <div style={{ padding: '64px 56px', borderRight: '1px solid rgba(20,18,16,0.08)' }}>
          <FadeUp>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div>
                <label style={label}>Nombre <span style={{ color: '#52448a' }}>*</span></label>
                <input style={inp} placeholder="Tu nombre" value={form.nombre} onChange={e => set('nombre', e.target.value)} />
              </div>
              <div>
                <label style={label}>Apellido <span style={{ color: '#52448a' }}>*</span></label>
                <input style={inp} placeholder="Tu apellido" value={form.apellido} onChange={e => set('apellido', e.target.value)} />
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }}>
              <div>
                <label style={label}>Email <span style={{ color: '#52448a' }}>*</span></label>
                <input style={inp} type="email" placeholder="correo@empresa.com" value={form.email} onChange={e => set('email', e.target.value)} />
              </div>
              <div>
                <label style={label}>Celular <span style={{ color: '#52448a' }}>*</span></label>
                <input style={inp} placeholder="+56 9 XXXX XXXX" value={form.celular} onChange={e => set('celular', e.target.value)} />
              </div>
            </div>
            ...
            {status === 'ok' && (
              <div style={{ padding: '20px 24px', background: '#eef4f0', borderLeft: '3px solid #2c4a3d', marginBottom: 20 }}>
                <p style={{ fontSize: 14, fontWeight: 400, color: '#2c4a3d', margin: 0 }}>✓ ¡Gracias! Tu mensaje fue enviado correctamente. Te contactaremos pronto.</p>
              </div>
            )}
            {status === 'error' && (
              <div style={{ padding: '20px 24px', background: '#fdf0f0', borderLeft: '3px solid #e05252', marginBottom: 20 }}>
                <p style={{ fontSize: 14, fontWeight: 400, color: '#c0392b', margin: 0 }}>Por favor completa todos los campos obligatorios.</p>
              </div>
            )}
            ...
            <div style={{ marginTop: 40, padding: '28px 32px', background: '#ece9e3', borderLeft: '3px solid rgba(82,68,138,0.3)' }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 300, lineHeight: 1.8, color: '#5e5850', margin: 0 }}>
                Tambien puedes llamarnos o escribirnos directamente. Dejanos un mensaje con tu nombre, numero de celular, region e interes — revisamos mensajes una vez al dia y te respondemos a la brevedad.<br />
        ...
        <div style={{ padding: '64px 48px', background: '#fff' }}>
          <FadeUp delay={0.1}>
            <div style={{ marginBottom: 36, paddingBottom: 36, borderBottom: '1px solid rgba(20,18,16,0.06)' }}>
              ...
              {[
                { icon: '📍', label: 'Direccion', val: 'Suecia 283, Providencia, Santiago, Chile' },
                { icon: '📞', label: 'Telefono', val: '+56 9 8413 6893' },
                { icon: '✉️', label: 'Email', val: 'contacto@furo.company' },
              ].map(i => (
                <div key={i.label} style={{ display: 'flex', gap: 14, marginBottom: 14, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <p style={{ fontSize: 10, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#8a8278', marginBottom: 3 }}>{i.label}</p>
                    <p style={{ fontSize: 14, fontWeight: 300, color: '#141210' }}>{i.val}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginBottom: 36, paddingBottom: 36, borderBottom: '1px solid rgba(20,18,16,0.06)' }}>
              ...
              {[
                { icon: '📍', label: 'Direccion', val: 'San Pablo 9490, Pudahuel, Santiago de Chile' },
                { icon: '🕐', label: 'Visitas', val: 'Lun – Vie, 9:00 – 17:00\nPrevia coordinacion' },
              ].map(i => (
                <div key={i.label} style={{ display: 'flex', gap: 14, marginBottom: 14, alignItems: 'flex-start' }}>
                  <div style={{ width: 34, height: 34, background: '#f5f3ee', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 14 }}>{i.icon}</div>
                  <div>
                    <p style={{ fontSize: 10, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.12em', color: '#8a8278', marginBottom: 3 }}>{i.label}</p>
                    <p style={{ fontSize: 14, fontWeight: 300, color: '#141210', whiteSpace: 'pre-line' }}>{i.val}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginBottom: 36, paddingBottom: 36, borderBottom: '1px solid rgba(20,18,16,0.06)' }}>
              ...
              <a href="https://wa.me/56977441963" target="_blank"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#25D366', color: '#fff', padding: '13px 22px', fontSize: 13, fontWeight: 500, fontFamily: 'DM Sans, sans-serif', textDecoration: 'none' }}>
      ...
      {/* INSTAGRAM */}
      <section style={{ padding: '72px 56px 80px', borderTop: '1px solid rgba(20,18,16,0.08)' }}>
        <FadeUp>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
            <div>
              ...
              <h2 style={{ fontSize: 30, fontWeight: 300, color: '#141210', margin: 0 }}>@furo.company</h2>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 2 }}>
            {[1,2,3,4,5].map(i => (
              <a key={i} href="https://instagram.com/furo.company" target="_blank"
                style={{ aspectRatio: '1', background: '#ddd8d0', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', cursor: 'pointer' }}
