...
const pasos = [
  {
    num: '01',
    titulo: 'Integracion temprana, una mejor toma de decisiones',
    texto: 'Nos involucramos desde el anteproyecto, entregando ingenieria, modelacion y analisis de costos, lo que permite una toma de decisiones mas certera desde el origen de cada proyecto.',
  },
  {
    num: '02',
    titulo: 'Modelacion y calculo para optimizar la ejecucion desde el origen',
    texto: 'Utilizamos tecnologia BIM modelando los proyectos en Revit, con detalles para la fabricacion de cada pieza y calculo estructural preciso, de esta manera optimizamos la ejecucion desde la etapa de diseno.',
  },
  {
    num: '03',
    titulo: 'Fabricacion precisa de componentes estructurales',
    texto: 'Contamos con una planta central de mas de 2.500 m², donde fabricamos con precision los componentes estructurales para cada proyecto. Estos elementos versatiles permiten soluciones en muros, pisos ventilados, entrepisos y cubiertas, alcanzando longitudes de hasta 12 metros sin uniones intermedias. Nuestra solucion de componentes estructurales incorporan la aislacion, los emplacados y todos los herrajes y fijaciones necesarios para su montaje.',
  },
  {
    num: '04',
    titulo: 'Eficientes y flexibles en la logistica',
    texto: 'Desarrollamos una logistica eficiente y flexible. Las piezas estan optimizadas para su transporte despiezado, incluso en zonas remotas, gracias a su ligereza y modularidad.',
  },
  {
    num: '05',
    titulo: 'Montaje agil y organico y acompanamiento en todo el proceso',
    texto: 'Nuestra metodologia de montaje es un proceso agil y organico, lo que nos permite reducir los tiempos en obra en un 40%, incluso en ubicaciones aisladas. Permite precision, disminuyendo imprevistos y el uso de mano de obra no especializada, lo que garantiza su escalabilidad y asequibilidad.',
  },
]
    ...
    <div>
      {pasos.map((p, i) => (
        <div key={p.num} style={{ borderTop: '1px solid rgba(20,18,16,0.1)' }}>
          <button onClick={() => setAbierto(abierto === i ? null : i)} style={{
            width: '100%', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
            padding: '28px 0', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', gap: 24
          }}>
            <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
              <span style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.15em', color: '#52448a', minWidth: 28, paddingTop: 3 }}>{p.num}</span>
              <span style={{ fontSize: 18, fontWeight: 400, lineHeight: 1.3, color: '#141210', fontFamily: 'DM Sans, sans-serif' }}>{p.titulo}</span>
            </div>
          ...
          <div style={{ maxHeight: abierto === i ? 300 : 0, overflow: 'hidden', transition: 'max-height 0.5s cubic-bezier(0.16,1,0.3,1)' }}>
            <p style={{ fontSize: 15, fontWeight: 300, lineHeight: 1.8, color: '#5e5850', paddingLeft: 52, paddingBottom: 28 }}>{p.texto}</p>
          </div>
        </div>
      ))}
  ...
  return (
    <main style={{ backgroundColor: '#f5f3ee', color: '#141210', fontFamily: 'DM Sans, sans-serif', minHeight: '100vh' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '100vh' }}>
        <div style={{ position: 'relative', overflow: 'hidden', background: '#ccc8bf', minHeight: 600 }}>
          <img src="/como-funciona/hero.jpg" alt="Como funciona" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0 }} />
        </div>
        <div style={{ padding: '140px 72px 80px 64px', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: '#fff' }}>
          <FadeUp>
            <span style={{ fontSize: 14, fontWeight: 500, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#52448a', display: 'block', marginBottom: 24 }}>Como funciona</span>
          </FadeUp>
          <RevealText tag="h1" delay={0.1} style={{ fontSize: 'clamp(32px,4vw,52px)', fontWeight: 300, lineHeight: 1.1, color: '#141210', marginBottom: 32 }}>
            Un enfoque inteligente a la manera de construir, smart timber.
          </RevealText>
          <FadeUp delay={0.2}>
            <p style={{ fontSize: 16, fontWeight: 300, lineHeight: 1.8, color: '#5e5850', marginBottom: 24 }}>
              Analizamos y modelamos los proyectos de manera precisa, fabricamos y construimos estructuras para desarrollos inmobiliarios, proyectos comerciales, colegios y volumenes que necesitan espacialidad, habitabilidad y eficiencia estructural.
            </p>
            <p style={{ fontSize: 16, fontWeight: 300, lineHeight: 1.8, color: '#5e5850', marginBottom: 40 }}>
              En un montaje agil — una construccion tipo lego o mecano — se ensamblan los componentes constituyendo una obra gruesa con menores imprevistos, tiempos de construccion y costos, y una mayor libertad en los espacios.
            </p>
      ...
      {/* ACORDEON */}
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#f5f3ee' }}>
        <div style={{ padding: '100px 64px 100px 56px' }}>
          <FadeUp>
            <Acordeon />
          </FadeUp>
        </div>
        <div style={{ padding: '100px 72px 100px 64px', display: 'flex', flexDirection: 'column', justifyContent: 'center', borderLeft: '1px solid rgba(20,18,16,0.08)' }}>
          <FadeUp delay={0.1}>
            <p style={{ fontSize: 16, fontWeight: 300, lineHeight: 1.8, color: '#5e5850', marginBottom: 24 }}>
              Analizamos y modelamos los proyectos de manera precisa, fabricamos y construimos estructuras para desarrollos inmobiliarios, proyectos comerciales, colegios y volumenes que necesitan espacialidad, habitabilidad y eficiencia estructural.
            </p>
            <p style={{ fontSize: 16, fontWeight: 300, lineHeight: 1.8, color: '#5e5850' }}>
              En un montaje agil — una construccion tipo lego o mecano — se ensamblan los componentes constituyendo una obra gruesa con menores imprevistos, tiempos de construccion y costos, y una mayor libertad en los espacios.
            </p>
          </FadeUp>
        </div>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', minHeight: '40vh', background: '#141210' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '80px 56px', display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: '40vh' }}>
          <FadeUp>
            <h2 style={{ fontSize: 'clamp(28px,3.5vw,44px)', fontWeight: 300, lineHeight: 1.2, color: '#fff', maxWidth: 480, marginBottom: 32 }}>
              Nuestra metodologia se adapta a los desafios de cada proyecto
            </h2>
      ...
      {/* COMPARADOR */}
      <section style={{ background: '#e8e5df', padding: '80px 56px' }}>
        <FadeUp>
          ...
          <div style={{ maxWidth: 700, margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 300, color: '#8a8278' }}>Construccion tradicional</span>
            ...
            <p style={{ textAlign: 'center', fontSize: 15, fontWeight: 300, color: '#5e5850' }}>
              disminuimos en un <strong style={{ color: '#52448a', fontWeight: 600 }}>40%</strong> el tiempo de montaje de la obra gruesa
            </p>
          </div>
        </FadeUp>
      </section>
          ...
          {componentes.map((comp) => (
            <div key={comp.label} style={{ position: 'relative', aspectRatio: '1/2', overflow: 'hidden', cursor: 'pointer' }}>
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px 14px' }}>
                <p style={{ fontSize: 11, fontWeight: 500, color: '#fff', lineHeight: 1.3 }}>{comp.label}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ background: '#fff', paddingTop: 60, textAlign: 'center' }}>
        <FadeUp>
          <h2 style={{ fontSize: 'clamp(26px,4vw,44px)', fontWeight: 300, lineHeight: 1.15, color: '#3a2d5e', maxWidth: 820, margin: '0 auto 58px', padding: '0 56px' }}>
