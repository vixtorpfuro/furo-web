...
export default function Nav({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', zIndex: 100, boxSizing: 'border-box', width: '100%' }}>
      ...
      <div className="nav-mobile-menu" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 99, background: 'rgba(20,18,16,0.97)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 8, opacity: menuOpen ? 1 : 0, pointerEvents: menuOpen ? 'all' : 'none', transition: 'opacity 0.35s cubic-bezier(0.16,1,0.3,1)', overflowX: 'hidden' }}>
        {links.map((item, i) => (
