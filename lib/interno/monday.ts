...
export async function getMisLeads(mondayPersonId: string): Promise<LeadResumen[]> {
  const query = `
    query ($boardId: ID!, $personId: ID!) {
      boards(ids: [$boardId]) {
        items_page(limit: 100, query_params: { rules: [{ column_id: "multiple_person_mm6d63rj", compare_value: [$personId], operator: any_of }] }) {
          items {
            id
            name
            created_at
            column_values(ids: ["lead_status", "long_text_mm6d9214"]) { id text }
          }
        }
      }
    }
  `
  const data = await mondayFetch(query, { boardId: BOARD_ID, personId: mondayPersonId })
  const items = data.boards?.[0]?.items_page?.items || []
  return items.map((item: { id: string; name: string; created_at: string; column_values: { id: string; text: string }[] }) => {
